...
set.seed(11882)
qplot((180 / pi) * rgamma(5000, 5, 300))
...
Que se ve como sigue para un valor de $\sigma_{dist} = 0.005$:
...
Este valor es probablemente muy grande, pues la probabilidad a 5 metros baja
a 50% por fallas en la distancia.

Ponemos una distribución inicial para $\sigma_d$. Esta cantidad
...
$$\sigma_{dist} \sim Gamma(10, 3000)I(\sigma_{dist} < 0.007)$$
```{r}
qplot(rgamma(10000, 5, 3000) %>% keep(~ .x < 0.007))
```
...
Finalmente, necesitamos hacer una observación acerca de los datos: como se trata
de cubetas, y no distancias exactas, esperamos que haya algo de sobredispersión
de los datos observados con respecto a la binomial. Esto podemos incorporarlo al
modelo considerando la probabilidad de éxito que calculamos como base, y luego
tomar
$$p_{sobre}(x) \sim Beta\left (p(x), \kappa\right)$$
donde usamos la parametrización de proporción y tamaño de muestra. Usamos $\kappa$
relativamente grande

```{r}
p <- 0.5
kappa <- 5000
qbeta(c(0.01, 0.99), p*kappa, (1-p)*kappa)
```

y ponemos $$\kappa \sim U(1000, 100000)$$
...
ensemble <- simular_ensemble(modelo_inicial, sim_ensemble_datos, 5000)
```

Y examinamos las siguientes probabilidades de éxito:

```{r}
exitos_tbl <- curvas_exito(ensemble, sim_ensemble_datos, 5000)
...
¿Cómo se ven las curvas de éxito para ángulo y distancia?

```{r}
prob_tbl <- curvas_prob_exito(ensemble, sim_ensemble_datos, 5000)
ggplot(prob_tbl, aes(x = x, y = value, group = .iteration)) + geom_line(alpha = 0.1) +
  facet_wrap(~variable)
```
...
No tuvimos problemas numéricos:
...
```{r}
prior_sd_sigma <- sd(rgamma(5000, 5, 3000) %>% keep(~ .x < 0.007))
calib_inf <- ajustes_ensemble %>%
  unnest(cols = (params)) %>% 
...
golf_grande <- read_delim("../datos/golf_grande.csv", delim = "\t") %>% 
  mutate(x = dis * 30.48, y = exitos, n = count) %>% 
  select(x, n, y) 
```
```{r}
golf_grande_2 <- golf_grande %>% mutate(prop_exito = y / n) %>% 
  mutate(bucket_size = x - lag(x))
ggplot(golf_grande_2 %>% filter(x < 1000), aes(x = x, y = n, size = bucket_size)) + geom_point() 
```
...
```{r}
golf_grande <- filter(golf_grande, x > 10)
```
...
g_post <- ggplot(pred_check_tbl, aes(x= x)) +
  geom_linerange(aes(ymin = q_05, ymax = q_95), size = 2) +
  geom_point(aes(y = y/n), colour="red", size = 2) +
  ylim(c(0,1)) + xlab("Distancia (cm)") + ylab("Tasa de éxito") #+
  #geom_point(data = datos, aes(y = y/n), colour = "blue") 
g_post 
```
Este modelo tiene algunos desajustes términos de la posterior, pero no son muy
grandes. Las desviaciones de las medias posteriores son muy chicas.

```{r}
ajuste_grande$summary(c("sigma_ang", "sigma_dist"))
```
```{r}
sim_inicial <- ensemble$draws(c("sigma_ang", "sigma_dist")) %>% as_draws_df() %>% 
  mutate(distrib = "inicial")
sim_posterior <- ajuste_grande$draws(c("sigma_ang", "sigma_dist")) %>% as_draws_df() %>% 
  mutate(distrib = "posterior")
sims <- bind_rows(sim_inicial, sim_posterior)
ggplot(sims,
       aes(x = (180/pi)*sigma_ang, y = sigma_dist, colour = distrib)) +
  geom_point(alpha = 0.5) 
```
Las probabilidades individuales de falla son:

```{r}
prob_tbl <- curvas_prob_exito(ajuste_grande, golf_grande, 16000)
ggplot(prob_tbl, aes(x = x, y = value, colour = variable, group = interaction(.iteration, .chain))) + 
  geom_line(alpha = 0.1) + scale_x_log10()
...
- La introducción de variabilidad para en la probabilidad de cada cubeta está
hecha para que el ajuste sea más apropiado. Sin embargo, es importante entender
su causa.
